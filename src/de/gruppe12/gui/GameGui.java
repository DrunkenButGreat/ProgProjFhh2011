...
public class GameGui extends JFrame {
	 ...
	 * GameGui Konstruktor
	 * 
	 * erzeugt GuiController und setzt die final Attribute cardLO und
	 * cardLOContainer
	 * 
	 */
	public GameGui() {
		...
		Font tempfont;
		try {
			tempfont = Font.createFont(Font.TRUETYPE_FONT, getClass()
					.getResourceAsStream("images/Viking_n.ttf"));
		} catch (Exception e) {
	 ...
	 * initGUI
	 * 
	 * setzt Fensterstartgroesse, ruft Komponenten bauende Methoden auf und f�gt
	 * diese anschlie�end im CardLayout zusammen
	 * 
	 */
	 ...
	 * buildStartMenu
	 * 
	 * baut das StartMenu aus vonwoaus zwischen den 3 Spielmodi gewaehlt werden
	 * kann.
	 * 
	 */
	...
	private void buildStartMenu() {
		...
		// jbtnHumanVsHuman.setBorder(BorderFactory.createMatteBorder(5, 5, 5,
		// 5, Color.DARK_GRAY));
		...
		/*
		 * sorgt bei Groessenaenderung dafuer, dass Schritfgroessen angepasst
		 * werden
		 */
	 ...
	 * buildHumanVsHuman
	 * 
	 * baut das Mensch-gegen-Mensch Auswahlmenue aus, wo die beiden Spieler zur
	 * Identifikation einen Namen eingeben koennen
	 */
	 ...
	 * buildHumanVsAi
	 * 
	 * baut das Mensch-gegen-KI Auswahlmenue, wo der Spieler einen Namen
	 * eingeben und zwischen den vorhandenen Gegner KIs waehlen kann.
	 * 
	 * 2 verknuepfte JRadioButtons dienen zur Auswahl, ob Spieler oder KI die
	 * Rolle des Angreifers uebernehmen
	 */
	...
	private void buildHumanVsAi() {
		...
		jbtnStart.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				String stratPath = controller.getKiFolderList().get(
						stratShortPath);
				Map<String, String> moveStrategies = controller
						.getStrats(stratPath);
		...
		jcbAiPath.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				String aiPath = controller.getKiFolderList().get(
						(String) jcbAiPath.getSelectedItem());
	...
	private void buildAiVsAi() {
		...
		jpnlHumanVsAi.addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				...
				jbtnStart.setFont(font.deriveFont(Font.BOLD, fontSize));
				jlbAiPath.setFont(font
						.deriveFont(Font.BOLD, fontSizeExtraSmall));
				jlbAiPath2.setFont(font.deriveFont(Font.BOLD,
						fontSizeExtraSmall));
				jcbDefenderAiPath.setFont(font.deriveFont(Font.BOLD,
						fontSizeExtraSmall));
				jcbOffenderAiPath.setFont(font.deriveFont(Font.BOLD,
						fontSizeExtraSmall));
			}
		});
		...
		jbtnStart.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				String shortOffStratPath = (String) jcbOffenderAiPath
						.getSelectedItem();
				String offStratPath = controller.getKiFolderList().get(
						shortOffStratPath);
				Map<String, String> offMoveStrategies = controller
						.getStrats(offStratPath);
				String offStrat = offMoveStrategies.get(offStratShort);

				String shortDefStratPath = (String) jcbDefenderAiPath
						.getSelectedItem();
				String defStratPath = controller.getKiFolderList().get(
						shortDefStratPath);
				Map<String, String> defMoveStrategies = controller
						.getStrats(defStratPath);
				String defStrat = defMoveStrategies.get(defStratShort);

				cardLO.show(cardLOContainer, cardNameGamePanel);
				controller.initAvAGame(offStratPath, offStrat, defStratPath,
						defStrat);
				jpnlBoardDisplay.resetSelectedCell();
				update();
			}
		...
		jcbOffenderAiPath.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				String aiPath = controller.getKiFolderList().get(
						(String) jcbOffenderAiPath.getSelectedItem());
				for (String s : controller.getStrats(aiPath).keySet()) {
		...
		jcbDefenderAiPath.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				String aiPath = controller.getKiFolderList().get(
						(String) jcbDefenderAiPath.getSelectedItem());
				for (String s : controller.getStrats(aiPath).keySet()) {
	...
	private void buildGameInfo() {
		...
		jlstLog.setBorder(BorderFactory.createTitledBorder(
		...
		jtfCurrentPlayer.setBorder(BorderFactory.createTitledBorder(
				BorderFactory.createEtchedBorder(), "Turn:"));
		((TitledBorder) jtfCurrentPlayer.getBorder()).setTitleFont(font
				.deriveFont(12F));
	 ...
	 * buildMenuBar
	 * 
	 * baut die Menueleiste auf, die das Beenden des Spiels, das Starten eines
	 * neuen Spiels und die Auswahl der KI Jar Datei ermoeglicht
	 * 
	 */
	...
	private void buildMenuBar() {
		...
		jmiKiJarChooser.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					String filename = jfcKiJarChooser.getSelectedFile()
							.getPath();
		...
		/*
		 * JMenuItem jmiKiFolderChooser= new
		 * JMenuItem("OrdnerPfad hinzufuegen"); final JFileChooser
		 * jfcKiFolderChooser= new JFileChooser();
		 * jfcKiFolderChooser.setFileSelectionMode
		 * (JFileChooser.DIRECTORIES_ONLY);
		 * jmiKiFolderChooser.addActionListener(new ActionListener() {
		 * 
		 * @Override public void actionPerformed(ActionEvent arg0) { int
		 * returnVal= jfcKiFolderChooser.showOpenDialog(cardLOContainer); if
		 * (returnVal == JFileChooser.APPROVE_OPTION) { String filename=
		 * jfcKiFolderChooser.getSelectedFile().getPath(); String shortPath=
		 * controller.addKiPathName(filename); if (shortPath!=null) {
		 * jcbAiPath.addItem(shortPath); jcbOffenderAiPath.addItem(shortPath);
		 * jcbDefenderAiPath.addItem(shortPath); } } } });
	 ...
	 * redraw
	 * 
	 * wird von GuiController durch Observer-Methode update() aufgerufen und
	 * fuehrt zum Neuzeichnen des Spielbretts
	 */
	...
	protected void update() {
		...
		if (controller.isDefendersTurn())
			currentPlayer = "Defender (" + playerDefenderName + ")";
		else
			currentPlayer = "Attacker (" + playerAttackerName + ")";
	 ...
	 * addToGBPanel
	 * 
	 * Hilfsmethode die ein geordnetes Hinzufuegen von Komponenten zu JPanels
	 * ermoeglicht
	 * 
	 * @param x
	 *            : x-Position der Zelle
	 * @param y
	 *            : y-Position der Zelle
	 * @param width
	 *            : Anzahl an einzunehmenden Zellen (rechts von x-Position)
	 * @param height
	 *            : Anzahl an einzunehmenden Zellen (unterhalb von y-Position)
	 * @param weightx
	 *            : bestimmt, wie stark die Komponente bei Groessenaenderung des
	 *            Zielpanels in x-Richtung groessenangepasst werden soll
	 * @param weighty
	 *            : wie weightx fuer y-Richtung
	 * @param comp
	 *            : die hinzuzufuegende Komponente
	 * @param targetPnl
	 *            : das JPanel, in das die Komponente hinzugefuegt werden sollt,
	 *            sollte ein GridBagLayout benutzen
	 * 
	 */

	private static void addToGBPanel(int x, int y, int width, int height,
			double weightx, double weighty, Component comp, JPanel targetPnl) {
