...
public class StrategyLoader {
	...
	public static ArrayList<String> listContent(String uri, Boolean folder)
			throws IOException {
		if (!folder)
			return listContent(uri);
		...
		if (dir.isDirectory()) {
			for (File file : dir.listFiles()) {
				classNames.addAll(listContent(file.getAbsolutePath(),
						file.getName()));
			}
		}
	 ...
	 * 
	 * @param ar
	 *            : ArrayList zum durchsuchen
	 * @param filter
	 *            : Endung der Datei / In unserem Fall class
	 * @return : ArrayList nur mit der vorgegebenen Endung
	 */
	public static ArrayList<String> filterExtension(ArrayList<String> ar,
			String filter) {
	...
	public static MoveStrategy getStrategy(String path, String classname) {
		if (path.contains(classname)) {
			if (path.contains("de"))
				classname = path.substring(path.indexOf("de"), path.length())
						.replace("\\", "/");
			if (path.contains("com"))
				classname = path.substring(path.indexOf("com"), path.length())
						.replace("\\", "/");
			if (path.contains("org"))
				classname = path.substring(path.indexOf("org"), path.length())
						.replace("\\", "/");
		...
		try {
			...
			String binaryName = classname.substring(0,
					classname.lastIndexOf('.'));
			binaryName = binaryName.replace('/', '.');

			ClassLoader classLoader = new URLClassLoader(new URL[] { jarURL });

			return (MoveStrategy) classLoader.loadClass(binaryName)
					.newInstance();
	 ...
	 */
	static MoveStrategy getFromClassPath(String name)
